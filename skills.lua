...
function lvlParametrs()
    for i =1, #playerSkills do 
        ...
        if (masSkill.numb == 8 ) then 
            playerSkillParametrs.waveAt =0.1*math.log(masSkill.lvl,2)
        end
...
function textPar(i,x,y,scale)
    local fontWidth = font:getWidth(tostring(skillCostUpgrade[i]))
    love.graphics.print("COST "..tostring(skillCostUpgrade[i]),x+30*k,y+fontWidth-40*k,-3.14/2,scale*k,scale*k)
    if (i == 1 ) then 
